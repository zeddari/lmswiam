...
class SponsoringResourceIT {
    ...
    @Mock
    private SponsoringRepository sponsoringRepositoryMock;
    ...
    void getAllSponsoringsWithEagerRelationshipsIsEnabled() throws Exception {
        when(sponsoringRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSponsoringMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(sponsoringRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllSponsoringsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(sponsoringRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSponsoringMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(sponsoringRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
