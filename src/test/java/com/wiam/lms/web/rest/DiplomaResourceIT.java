...
class DiplomaResourceIT {
    ...
    private static final DiplomaType DEFAULT_TYPE = DiplomaType.PRIMARY;
    private static final DiplomaType UPDATED_TYPE = DiplomaType.SECONDARY;
    ...
    @Mock
    private DiplomaRepository diplomaRepositoryMock;
    ...
    void createDiploma() throws Exception {
        ...
        assertThat(testDiploma.getType()).isEqualTo(DEFAULT_TYPE);
        ...
        assertThat(testDiploma.getAttachment()).isEqualTo(DEFAULT_ATTACHMENT);
        assertThat(testDiploma.getAttachmentContentType()).isEqualTo(DEFAULT_ATTACHMENT_CONTENT_TYPE);
    }
    ...
    void getAllDiplomasWithEagerRelationshipsIsEnabled() throws Exception {
        when(diplomaRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restDiplomaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(diplomaRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllDiplomasWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(diplomaRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restDiplomaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(diplomaRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void putExistingDiploma() throws Exception {
        ...
        Diploma testDiploma = diplomaList.get(diplomaList.size() - 1);
        assertThat(testDiploma.getType()).isEqualTo(UPDATED_TYPE);
        ...
        assertThat(testDiploma.getAttachment()).isEqualTo(UPDATED_ATTACHMENT);
        assertThat(testDiploma.getAttachmentContentType()).isEqualTo(UPDATED_ATTACHMENT_CONTENT_TYPE);
        await()
            .atMost(5, TimeUnit.SECONDS)
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(diplomaSearchRepository.findAll());
                ...
                assertThat(testDiplomaSearch.getType()).isEqualTo(UPDATED_TYPE);
                ...
                assertThat(testDiplomaSearch.getAttachment()).isEqualTo(UPDATED_ATTACHMENT);
                assertThat(testDiplomaSearch.getAttachmentContentType()).isEqualTo(UPDATED_ATTACHMENT_CONTENT_TYPE);
            });
    }
