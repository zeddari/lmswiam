...
class SiteResourceIT {
    ...
    void getAllSitesWithEagerRelationshipsIsEnabled() throws Exception {
        when(siteRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSiteMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(siteRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void partialUpdateSiteWithPatch() throws Exception {
        ...
        partialUpdatedSite.nameLat(UPDATED_NAME_LAT).description(UPDATED_DESCRIPTION);
