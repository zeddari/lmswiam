...
public class PaymentResource {
    ...
    public ResponseEntity<Payment> partialUpdatePayment(
        ...
        Optional<Payment> result = paymentRepository
            ...
            .map(existingPayment -> {
                ...
                if (payment.getProof() != null) {
                    existingPayment.setProof(payment.getProof());
                }
                if (payment.getProofContentType() != null) {
                    existingPayment.setProofContentType(payment.getProofContentType());
                }
                ...
                if (payment.getSide() != null) {
                    existingPayment.setSide(payment.getSide());
                }
                if (payment.getValidityStartTime() != null) {
                    existingPayment.setValidityStartTime(payment.getValidityStartTime());
                }
                if (payment.getValidityEndTime() != null) {
                    existingPayment.setValidityEndTime(payment.getValidityEndTime());
                }
    ...
    public ResponseEntity<List<PaymentTransactionDTO>> getPaymentTransactions(
        ...
        List<PaymentTransactionDTO> transactions = paymentRepository.getPaymentTransactions(siteId, startDate, endDate);
