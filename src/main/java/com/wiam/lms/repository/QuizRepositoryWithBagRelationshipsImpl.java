...
public class QuizRepositoryWithBagRelationshipsImpl implements QuizRepositoryWithBagRelationships {
    ...
    public Optional<Quiz> fetchBagRelationships(Optional<Quiz> quiz) {
        return quiz.map(this::fetchGroups).map(this::fetchQuestions);
    }
    ...
    public List<Quiz> fetchBagRelationships(List<Quiz> quizzes) {
        return Optional.of(quizzes).map(this::fetchGroups).map(this::fetchQuestions).orElse(Collections.emptyList());
    }

    Quiz fetchGroups(Quiz result) {
        return entityManager
            .createQuery("select quiz from Quiz quiz left join fetch quiz.groups where quiz.id = :id", Quiz.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Quiz> fetchGroups(List<Quiz> quizzes) {
        ...
        List<Quiz> result = entityManager
            .createQuery("select quiz from Quiz quiz left join fetch quiz.groups where quiz in :quizzes", Quiz.class)
