...
public class SessionRepositoryWithBagRelationshipsImpl implements SessionRepositoryWithBagRelationships {
    ...
    Session fetchClassrooms(Session result) {
        return entityManager
            .createQuery("select session from Session session left join fetch session.classrooms where session.id = :id", Session.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Session> fetchClassrooms(List<Session> sessions) {
        ...
        List<Session> result = entityManager
            .createQuery("select session from Session session left join fetch session.classrooms where session in :sessions", Session.class)
    ...
    Session fetchGroups(Session result) {
        return entityManager
            .createQuery("select session from Session session left join fetch session.groups where session.id = :id", Session.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Session> fetchGroups(List<Session> sessions) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, sessions.size()).forEach(index -> order.put(sessions.get(index).getId(), index));
        List<Session> result = entityManager
            .createQuery("select session from Session session left join fetch session.groups where session in :sessions", Session.class)
            .setParameter("sessions", sessions)
            .getResultList();
        Collections.sort(result, (o1, o2) -> Integer.compare(order.get(o1.getId()), order.get(o2.getId())));
        return result;
