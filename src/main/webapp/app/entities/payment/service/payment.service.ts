...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'paidAt' | 'validityStartTime' | 'validityEndTime'> & {
  paidAt?: string | null;
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...
      validityStartTime: payment.validityStartTime?.toJSON() ?? null,
      validityEndTime: payment.validityEndTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...
      validityStartTime: restPayment.validityStartTime ? dayjs(restPayment.validityStartTime) : undefined,
      validityEndTime: restPayment.validityEndTime ? dayjs(restPayment.validityEndTime) : undefined,
    };
  }
