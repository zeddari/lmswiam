...
type RestOf<T extends IUserCustom | NewUserCustom> = Omit<T, 'birthdate'> & {
...
export class UserCustomService {
  ...
  protected convertDateFromClient<T extends IUserCustom | NewUserCustom | PartialUpdateUserCustom>(userCustom: T): RestOf<T> {
    return {
      ...userCustom,
      birthdate: userCustom.birthdate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUserCustom: RestUserCustom): IUserCustom {
    return {
      ...restUserCustom,
      birthdate: restUserCustom.birthdate ? dayjs(restUserCustom.birthdate) : undefined,
    };
  }
