...
type RestOf<T extends ITickets | NewTickets> = Omit<T, 'dateTicket' | 'dateProcess' | 'from' | 'toDate'> & {
...
export class TicketsService {
  ...
  protected convertDateFromClient<T extends ITickets | NewTickets | PartialUpdateTickets>(tickets: T): RestOf<T> {
    return {
      ...
      from: tickets.from?.toJSON() ?? null,
      toDate: tickets.toDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTickets: RestTickets): ITickets {
    return {
      ...
      from: restTickets.from ? dayjs(restTickets.from) : undefined,
      toDate: restTickets.toDate ? dayjs(restTickets.toDate) : undefined,
    };
  }
